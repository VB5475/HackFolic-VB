...
function MainListItem({ cardData, pageName, handleUpdateFlag }) {
 ...
 function handleAccept(id) {
  ...
  const result = updateApplication(
   `https://hackfolic-vb-8.onrender.com/hackathonApplications/${id}`,
 ...
 function handleDelete(id, cardRole) {
  ...
  if (cardRole === "participant") {
   result = deleteUser(
    `https://hackfolic-vb-8.onrender.com/participants/${id}`
   );
  } else if (cardRole === "host") {
   result = deleteUser(`https://hackfolic-vb-8.onrender.com/hosts/${id}`);
   console.log(result);
  }
  if (result.success) {
   handleUpdateFlag();
