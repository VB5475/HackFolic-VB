  ...
  const fetchData = async () => {
   try {
    const result = await getApplication(
     `https://hackfolic-vb-8.onrender.com/hackathonApplications/${applicationId}`
    );
    console.log(result);
    if (result.success) {
     console.log();
  ...
  <div className="   text-start h-[100vh] flex flex-col items-center justify-center text-2xl p-2 ">
   <div className=" w-ful md:w-auto p-10 shadow-md shadow-blue-500 rounded-md">
    <div className="font-bold">
     Leader Name: <span className="font-normal"> {leaderName}</span>
    </div>
    <p className="font-bold">
     Problem Statement: <span className="font-normal">{problemStatement}</span>{" "}
    </p>
    <div>
     Team Details:
     {teamDetails && teamDetails.length > 0
      ? teamDetails.map((teamMember, index) => (
         <>
          <div className="font-bold">
           Team Member {index + 1} :{" "}
           <span className="font-normal">{teamMember.firstName}</span>
          </div>
          <div className="font-bold">
           Member Email :{" "}
           <span className="font-normal">{teamMember.email}</span>{" "}
          </div>
         </>
        ))
